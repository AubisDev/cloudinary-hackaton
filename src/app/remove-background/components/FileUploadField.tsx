...
import ImageEdit from './Instructions/ImageEdit';
...
const FileUploadField = () => {
  ...
  const onDrop = useCallback( async (acceptedFiles:File[], rejectedFiles: FileRejection[]) => {
    if( acceptedFiles.length === 1 ){
      ...
      setLoading(true);
      return await axios.post("https://api.cloudinary.com/v1_1/djm3yrljp/image/upload", formData, {
        headers: {"X-Requested-With": "XMLHttpRequest"}
      })
      .then((response) => {
          const data = response.data;
          const publicId = data.public_id;
          const imageWithoutBackground = cld.image(publicId).effect(backgroundRemoval());
  ...
  return (
    <section className="w-[50%] flex justify-center flex-col ">
      ...
      >
        <input {...getInputProps()} />
        <strong>Drag 'n' drop some images here, or click to select images</strong>
...

      {
        image.length !== 0 ? 
        (
          <Link href={image} target="_blank" className="w-1/3 py-2 m-auto mt-5 font-semibold text-center text-white bg-green-700 rounded-lg hover:bg-green-800">
            Get full size image
          </Link>
        ) : null
      } 
